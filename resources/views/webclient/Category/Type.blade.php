...
@section('content')
                ...
                <div class="col-sm-24">
                    <div class="breadcrumbs">
                        <ul>
                            ...
                            <li class="contact"> <strong>  {{$Type_name->type_name}}</strong>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-sm-18 col-sm-push-6 em-col-main">
                            ...
                            <div class="category-products">
                                ...
                                <div id="em-grid-mode">
                                    <ul class="emcatalog-grid-mode products-grid emcatalog-disable-hover-below-mobile" id="showlbook">
                                        ...
                                        <dd>
                                            <ol class="filter tree-filter">
                                                @foreach($ListType as $item)
                                                    @if($item->category_id == $Type_name->category_id)
                                                        <li>
                                                            <div class="label-icon">
                                                                <div class="label"> <a class="tree-item" href="/Product/Type/{{$item->type_id}}">{{$item->type_name}}</a></div>
                                                            </div>
                                                        </li>
                                                    @endif
                                                @endforeach
    ...
    <script>
        $('document').ready(function () {
            $('#toolbar-show').change(function () {
                var typeid = {{$Type_name->type_id}}+"";
                var quantity = $(this).val();
                var typesort = $('#sortby').val();
                $.get("/ajax/type/showby/"+typesort+"/"+typeid+"/"+quantity,function (data) {
                    $('#showlbook').html(data);
                });
            });
        });
        $('document').ready(function () {
            $('#sortby').change(function () {
                ...
                window.location="/Product/Type/"+typeid+"?order="+type+"&from="+from+"&to="+to+"&sort="+sort;
            });
        });
        $('document').ready(function () {
